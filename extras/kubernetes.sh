seal() {
	echo kubeseal --scope=$KUBESEAL_SCOPE --raw --from-file=/dev/stdin --cert=$KUBESEAL_CERT
	echo -n $1 | kubeseal --scope=$KUBESEAL_SCOPE --raw --from-file=/dev/stdin --cert=$KUBESEAL_CERT
}
sealnc() {
	echo -n $1 | kubeseal --scope=$KUBESEAL_SCOPE --raw --from-file=/dev/stdin --controller-name=sealed-secrets --controller-namespace=kube-system
}
sealsecret() {
	kubeseal --format=yaml --cert="$KUBESEAL_CERT" --scope="$KUBESEAL_SCOPE" <"$1"
}
kgp() {
	if [ $2 ]; then
